...
describe('4G performance audit via Lighthouse', () => {
  ...
  test('first contentful paint should score 90+', () => {
    expect(lh.audits['first-contentful-paint'].score).toBeGreaterThanOrEqual(
      0.9
    );
  });

  test('time to interactive should score 90+', () => {
    expect(lh.audits['interactive'].score).toBeGreaterThanOrEqual(0.9);
  });

  test('first cpu idle should score 90+', () => {
    expect(lh.audits['first-cpu-idle'].score).toBeGreaterThanOrEqual(0.9);
  });

  test('js boot-up should score 90+', () => {
    expect(lh.audits['bootup-time'].score).toBeGreaterThanOrEqual(0.9);
  });

  test('mainthread work should score 90+', () => {
    expect(lh.audits['mainthread-work-breakdown'].score).toBeGreaterThanOrEqual(
      0.9
    );
  });
  ...
  test('user timing:your-user-timing should be less than 1000ms', () => {
    expect(
      lh.audits['user-timings'].details.items.find(
        (item) => item.name === 'your-user-timing'
      ).startTime
    ).toBeLessThan(1000);
  });
});
